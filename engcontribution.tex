...
The proposed algorithm has achieved the objectives we had enumerated in the section \ref{intro:objetivos}. A novel technique has been proposed that allows adapting interactively the virtual patient anatomy with internal tissues from their original position to any desired pose. Furthermore, the algorithm will work with incomplete anatomical data an without a mechanical characterisation of the patient tissues. %This is possible even the models are incomplete or a proper mechanical description is not available. %Revisar
...
The main limitation of the proposed algorithm is to use a geometrically-based approach. That means, it generates a heuristic solution, and it is not physically correct. For this reason, this technique is not suitable for surgical planning or applications which need a realistic behaviour. For example, our algorithm does not take into account the effect of gravity and produces the same deformation no matter if the patient is standing up or lying.
